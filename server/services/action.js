...
const actions = {
    ...
    WORKFLOW: async ({ caseId, stageId, form, user }) => {
        ...
        try {
            if (form.data && form.data['CaseNote_AllocationNote']) {
                await caseworkServiceClient.post(`/case/${caseId}/note`, { type: 'TRANSITION', text: form.data['CaseNote_AllocationNote'] }, headers);
            }
            if (form.data && form.data['CaseNote_RejectionNote']) {
                await caseworkServiceClient.post(`/case/${caseId}/note`, { type: 'REJECT', text: form.data['CaseNote_RejectionNote'] }, headers);
            }
